...
function xhrGetWithHeader(url,headerTemp, callback, errback){
	var xhr = new createXHR();
	xhr.open("GET", url, true);
	//xhr.setRequestHeader("Authorization", "Bearer qvtran@us.ibm.comQllygd_1");
	xhr.setRequestHeader(""+ headerTemp.key,""+ headerTemp.value);
	xhr.onreadystatechange = function(){
		if(xhr.readyState == 4){
			if(xhr.status == 200){
				callback(parseJson(xhr.responseText));
